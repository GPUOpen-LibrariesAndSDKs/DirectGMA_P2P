...
public:
    ...
    bool            createDownStream(unsigned int w, unsigned int h, int nIntFormat, int nExtFormat, int nType, bool bUseP2P);

    void            draw();

    void            release();

    AlignedMem*         getPinnedMemory();

    unsigned long long* getBufferBusAddress();
    unsigned long long* getMarkerBusAddress();

    BufferQueue*       getInputBuffer() { return m_pSyncBuffer; };
