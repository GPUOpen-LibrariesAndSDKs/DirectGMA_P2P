...
public:
	...
	// set up ADL and enumerates Displays
	...
	static unsigned int getDisplayOnGPU(unsigned int uiGPU, unsigned int n);

	static DisplayData* getDisplayData(unsigned int uiDspId); 


	 // returns the number of GPUs in the system
    static unsigned int    getNumGPUs()        { return m_uiNumGPU; };

	// returns the number of displays mapped on GPU uiGPU
	static unsigned int getNumDisplaysOnGPU(unsigned int uiGPU);
...
private:
	...
	static bool setupADL(); // Load ADL library and get function pointers
	static void* __stdcall ADL_Alloc ( int iSize );
	static void __stdcall ADL_Free ( void* lpBuffer );

	

	static unsigned int                m_uiNumGPU;
	static std::vector<DisplayData*>  m_DisplayInfo;

	static ADLFunctions g_AdlCalls;
